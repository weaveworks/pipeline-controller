...
func ParseURL(provider string, repoURL string) (ParsedURL, error) {
	parsed := ParsedURL{}

	switch provider {
	case GitHubProviderName, GitLabProviderName:
		userRepoRef, err := gitprovider.ParseUserRepositoryURL(repoURL)
		if err != nil {
			return parsed, err
		}
		parsed.Domain = userRepoRef.Domain
		parsed.RepositoryName = userRepoRef.RepositoryName
	case BitBucketServerProviderName:
		orgRepoRef, err := GoGitProvider{}.ParseBitbucketServerURL(repoURL)
		if err != nil {
			return parsed, err
		}
		parsed.Domain = orgRepoRef.Domain
		parsed.OrganizationName = orgRepoRef.Organization
		parsed.RepositoryName = orgRepoRef.RepositoryName
	}
